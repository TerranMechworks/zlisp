...
use zlisp_text::{to_pretty, ErrorCode, WhitespaceConfig};

macro_rules! assert_unsupported {
    ($type:ty, $value:expr) => {
        let v: $type = $value;
        let err = to_pretty(&v, WhitespaceConfig::default()).unwrap_err();
...
fn string_tests() {
    ...
    let _ = to_pretty(&max_len, WhitespaceConfig::default()).unwrap();
